		...
		<div id="contatos">
			...
			<aside id="users_online">
				<ul>
					...
					?>
						<li id="<?php echo $_SESSION['id_user'] ?>:<?php echo $row->id; ?>" data-toggle="popover" data-content="<center><div class='imgSmall'><img src='img/perfis/<?php echo $row->image; ?>'/></div><h5 style='float:left;'><?php echo $row->name." ".$row->lastname; ?></center>">
					...
					<div class="panel panel-default" style="margin-top:1%;">
						...
						<div class="panel-footer" style='background:#fff;'>
							<h2 style="margin-top:0%;">
								<?php echo $name_perfil." ".$lastname_perfil; ?>
								<div class="actions">
									<?php
										if($_GET['id'] != $_SESSION['id_user']){
											if(Friends::renderFriendShip($_SESSION['id_user'],$_GET['id'], 'isThereRequestPending') == 1){
												echo "<button class='request_pending btn btn-default' disabled>Solicitação Pendente</button>";
											}else{
												if(Friends::renderFriendShip($_SESSION['id_user'],$_GET['id'],'isThereFriendShip') == 0){
													echo "<button class='friendBtn add btn btn-default' data-uid='".$_GET['id']."' onclick='add_friend' data-type='addfriend'>Enviar Solicitação de Amizade</button>";
													echo "<button class='request_pending btn btn-default hidden' disabled>Solicitação Pendente</button>";
												}else{
													echo "<button class='friendBtn unfriend btn btn-default' data-uid='".$_GET['id']."' data-type='unfriend'>Desfazer Amizade</button>";
													echo "<button class='friendBtn add btn btn-default hidden' data-uid='".$_GET['id']."' data-type='addfriend'>Enviar Solicitação de Amizade</button>";
